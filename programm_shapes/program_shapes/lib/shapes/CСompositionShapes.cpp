...
void C—ompositionShapes::CloseAllShapes()
{
	m_shapes.clear();
}
...
std::string C—ompositionShapes::GetCharacteristics()
{
...
void C—ompositionShapes::ApplyFill(sf::Vector2i pos)
{
    for (auto& [id, shape] : m_shapes)
    {
...
void C—ompositionShapes::ApplyOutlineColor(sf::Vector2i pos)
{
    for (auto& [id, shape] : m_shapes)
    {
        if (shape->IsClick(pos))
...
void C—ompositionShapes::ApplyOutlineThickness(sf::Vector2i pos)
{
    for (auto& [id, shape] : m_shapes)
    {
        if (shape->IsClick(pos))
...
std::shared_ptr<IMementoShapes> C—ompositionShapes::CreateMement()
{
    return std::make_shared<CompositionShapesMemento>(*this);
}

void C—ompositionShapes::SetCompositionShapes(std::map<int, std::shared_ptr<IShape>>& shapes)
{
    m_shapes.clear();
    m_curentShapeId = 0;
...
void C—ompositionShapes::ApplyVisitor(IShapeVisitor& visitor)
{
    for (auto& [id, shape] : m_shapes)
    {
...
void C—ompositionShapes::ApplyVisitorToClickedShape(const sf::Vector2i& pos, IShapeVisitor& visitor)
{
    for (auto& [id, shape] : m_shapes)
    {
        if (shape->IsClick(pos))
